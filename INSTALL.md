...
ignores unused imports). Then run the following command in the same directory
as `glide.yaml` to populate your /vendor directory.
...
requested by `client-go`. For example, if your project wants to use a newer
version of a package than `client-go`, modify `glide.yaml` to pin that package:
