...
func main() {
	...
	controller := NewController(queue, indexer, informer, func(cache.Indexer, workqueue.RateLimitingInterface) bool {
		...
		if err != nil {
			...
			// on invalid keys. Furhter let's now and then forget about the key, so that the queue
			// does not get too big in case the queue is flooded with a log of invalid keys.
			if queue.NumRequeues(key) < 20 {
...
func NewPanicCatcher() func() {
	return func() {
		if r := recover(); r != nil {
			glog.Fatalf("Panic with stacktrace %s", string(debug.Stack()))
		}
	}
}
